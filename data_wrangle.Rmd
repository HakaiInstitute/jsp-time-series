...
df <- sealice_time_series %>% 
  ...
  nest(-year, -region, -species, -louse_species) %>% 
  mutate(booted = map(data, ~ bootstraps(., times = 10))) %>% 
  unnest(booted)
...
```{r}
# Test chunk for bootstrap CIs (delete later once CIs are figured)
...
# Function to sample 4 of the 10 individuals in each of the 24 subsets (n_regions * n_species * n_sexes)
## and creates a new data fram with 96 cases and computes the mean of body size in each of the subsets.
## The whole thing is done 100 times and we end up with 100 bootstrapped means for the 24 subsets
...
cu_cum_abund_2017 <- cu_cum_abund_2017 %>% 
  mutate(species = "Chum")

# For now, I'll fjust use raw points without a line
write_csv(cu_cum_abund_annual_DI_2017, here("data", "cu_cum_abund_2017.csv"))
