...
```{r abundance bootstrapping attempts}
library(rsample)
library(dplyr)
library(tidyr)
library(broom)
...
# library(tidyverse)
# 
# sealice_time_series = read_csv('data/sealice_time_series.csv')
...
# # Slipper method
# #library(slipper)
# filepath='C:/Users/Biljana/Nextcloud/BrettJohnson/slipper/slipper-master/slipper-master/R'
# source(paste(filepath,'/slipper.R',sep=''))
# source(paste(filepath,'/slipper_ci.R',sep=''))
# source(paste(filepath,'/slipper_lm.R',sep=''))
# source(paste(filepath,'/utils.R',sep=''))
# library(lazyeval)                            
# 
# df2 <- sealice_time_series %>% 
#   filter(louse_species != "all_lice") %>% 
#   group_by(year, region, species, louse_species) %>% 
#   slipper(mean(n_lice))
...
this_is_it <- sealice_time_series %>% 
  select(year, region, species, louse_species, n_lice) %>%  
  filter(louse_species != "all_lice") %>% 
  split(list(.$region, .$year, .$species, .$louse_species)) 


boots <- lapply(1:length(this_is_it), function(x){bootstraps(this_is_it[[x]], times=10000)})
