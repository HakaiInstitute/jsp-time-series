...
lab_lice <- left_join(survey_seines, hakaisalmon::fish_field_data) %>%
...
field_lice <- left_join(survey_seines, hakaisalmon::fish_field_data) %>%
...
abundance <- sealice_time_series %>%
  ...
  group_by(year, region, species, louse_species) %>%
  summarise(abundance = mean(n_lice, na.rm = T), n = n(), abundance_sd = sd(n_lice))
