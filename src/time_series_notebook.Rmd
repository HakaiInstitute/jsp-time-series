...
In an effort to consider alternative methods to estimate run-timing I will
consider here I suppose another method to be to take the average and standard
deviations of each year and plot a normal distribution.
...
so_catch <- as.data.frame(tidy_catch)

so_catch_expanded <- so_catch[rep(row.names(tidy_catch), tidy_catch$n), 1:5] %>% 
...
ggplot(sim_set, aes(x = yday, y = year, fill = year)) +
         geom_density_ridges()+
  facet_grid(region ~ .)
...
# TODO redo this code so that it calculates CPUE by year, by region by species
# in a simple dplyr pipe chain

# Calculate cpue by region annualy
# DI
di_n_sockeye_per_year <- so_catch_expanded %>% 
  filter(region == "DI") %>% 
  group_by(year, seine_id) %>% 
  # count the number of fish from each seine
  summarise(n = n()) %>% 
  group_by(year) %>% 
  # calculate the annual mean cpue
  summarize(cpue = mean(n))

di_cpue_2015 <- as.numeric(di_n_sockeye_per_year[1,2])
di_cpue_2016 <- as.numeric(di_n_sockeye_per_year[2,2])
di_cpue_2017 <- as.numeric(di_n_sockeye_per_year[3,2])
# JS
JS_n_sockeye_per_year <- so_catch_expanded %>% 
  filter(region == "JS") %>% 
  group_by(year, seine_id) %>% 
  # count the number of fish from each seine
  summarise(n = n()) %>% 
...
JS_cpue_2015 <- as.numeric(JS_n_sockeye_per_year[1,2])
JS_cpue_2016 <- as.numeric(JS_n_sockeye_per_year[2,2])
JS_cpue_2017 <- as.numeric(JS_n_sockeye_per_year[3,2])
...
proportions <- tidy_catch_expanded %>% 
