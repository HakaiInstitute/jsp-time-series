...
library(hakaisalmon)
library(googledrive)
# This same data wrangling file should be used for the pdf report as well as the shiny app
f <- function(x) {
  format(x + as.Date("2018-01-01") -1, format = "%B %d")
}

# Downloads and unpacks the .zip of JSP Data v.1.0.2 from the Hakai Metadata Catalogue, DOI http://dx.doi.org/10.21966/1.566666 
drive_download(as_id("https://drive.google.com/open?id=1dSaGlM9Em6Lo9_jfL9qO7nnm8pdjMYNf"),
               path = here("data", "v1.0.2.zip"),
               overwrite = TRUE)


survey_data <- read_csv(unzip(here("data", "v1.0.2.zip"), files = "data/survey_data.csv", exdir = here("data", "v1.0.2"), overwrite = TRUE)) %>% 
...
seine_data <- read_csv(unzip(here("data", "v1.0.2.zip"), files = "data/seine_data.csv", exdir = here("data", "v1.0.2"), overwrite = TRUE))

survey_seines <- full_join(seine_data, survey_data, by = "survey_id") %>% 
