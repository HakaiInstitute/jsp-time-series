...
survey_seines_2019 <- full_join(seine_data, survey_data) %>% 
...
survey_seines_2015_2018 <- hakaisalmon::survey_seines
...
survey_seines_2015_2018 <- survey_seines_2015_2018 %>% 
  ...
  # TODO: Update these sites to inlcude 2019 sites
  filter(
    site_id %in% c(
      "D07",
      "D09",
  ...
  filter(yday < 190) %>% 
  # remove columns that were not used in 2019
  select(-time_searching)

# Join 2015-2018 wirh 2019 data
survey_seines <- full_join(survey_seines_2019, survey_seines_2015_2018)
...
sealice_field_data_2019 <- gs_read(field_2019, 
                                       ws = "sealice_field_data")

fish_and_sealice_field_data_2019 <- left_join(survey_seines_fish_2019, sealice_field_data_2019) %>%
  drop_na(ufn) %>% 
  ...
  # I think this correctly creates zeros where NAs are created by default
  drop_na(n_lice)

#### end 2019 update section
