...
sites <- read_csv(unzip(here("data", "v1.0.2.zip"), files = "data/sites.csv", exdir = here("data", "v1.0.2"), overwrite = TRUE))
...
fish_field_data <- read_csv(unzip(here("data", "v1.0.2.zip"), files = "data/fish_field_data.csv", exdir = here("data", "v1.0.2"), overwrite = TRUE), guess_max = 20000)
survey_seines_fish <- left_join(survey_seines, fish_field_data, by = "seine_id") %>% 
  drop_na(ufn)

sealice_field_data <- read_csv(unzip(here("data", "v1.0.2.zip"), files = "data/sealice_field.csv", exdir = here("data", "v1.0.2"), overwrite = TRUE))

# Summarize field counts of motile sea lice only
fish_and_sealice_field_data <- left_join(survey_seines_fish, sealice_field_data, by = "ufn") %>%
  filter(lice_id_protocol_field != "NA") %>%
  mutate(all_lice = cal_mot_field + cgf_field + lpam_field + lpaf_field + lam_field + laf_field + lgf_field,
         motile_caligus = cal_mot_field,
         motile_lep = lpam_field + lpaf_field + lam_field + laf_field + lgf_field) %>% 
...
sealice_lab_mot <- read_csv(unzip(here("data", "v1.0.2.zip"), files = "data/sealice_lab_mot.csv", exdir = here("data", "v1.0.2"), overwrite = TRUE))
sealice_lab_fs_mot <- read_csv(unzip(here("data", "v1.0.2.zip"), files = "data/sealice_lab_fs.csv", exdir = here("data", "v1.0.2"), overwrite = TRUE)) %>% 
...
field_lice <-
  left_join(survey_seines, fish_field_data, by = "seine_id") %>%
  left_join(sealice_field, by = "ufn") %>%
