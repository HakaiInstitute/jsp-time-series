...
# Currently 2019 field and lab data are not thoroughly QC'd and entered into our
# database; In the meantime we'll use the raw data from googlesheets.
# TODO: Replace all 2019 raw googlesheet data with QC'd JSP database data
  
# Add in 2019 field data from google sheet
...
survey_seines_2019 <- full_join(seine_data, survey_data, by = "survey_id") %>% 
  mutate(year = year(survey_date)) %>% 
  drop_na(survey_id) %>% 
  mutate(yday = yday(survey_date)) %>% 
  select(-contains("time")) %>% 
  # remove ad-hoc collections from migration timing calcs & sampling events forom normal sampling period (May 1 - July 9)
  filter(
...
# Import data from 2015-2018
survey_seines_2015_2018 <- left_join(read_csv("https://raw.githubusercontent.com/HakaiInstitute/jsp-data/master/data/survey_data.csv"), read_csv("https://raw.githubusercontent.com/HakaiInstitute/jsp-data/master/data/seine_data.csv")) %>% 
  left_join(read_csv("https://raw.githubusercontent.com/HakaiInstitute/jsp-data/master/data/sites.csv"))

survey_seines_2015_2018 <- survey_seines_2015_2018 %>% 
# The way time is stored in the data package files is in a different format from the raw data
  select(-contains("time")) %>% 
...
# Join 2015-2018 with 2019 data
survey_seines <- bind_rows(survey_seines_2019, survey_seines_2015_2018)

survey_seines$sampling_week <- recode_factor(survey_seines$sampling_week, 
                                           `18` = "May 5",
                                           `19` = "May 12" ,
                                           `20` = "May 19", 
                                           `21` = "May 26",
...
```{r sealice}
# Bring in 2019 raw data from google sheets to join
fish_field_data_2019 <- gs_read(field_2019, ws = "fish_field_data")
survey_seines_fish_2019 <- left_join(survey_seines_2019, fish_field_data_2019, by = "seine_id") %>% 
  drop_na(ufn)

# QC that only the proper fish are filtered out by the seine/survey restrictions in the setup chunk
# s <- fish_field_data_2019 %>% 
#   left_join(seine_data, by = "seine_id") %>%
#   left_join(survey_data, by = c("survey_id.x" = "survey_id")) %>% 
#   filter(!ufn %in% survey_seines_fish_2019$ufn) %>% 
#   group_by(seine_id, survey_date, site_id, set_type, survey_type, collection_protocol) %>% 
#   summarize(count=n()) %>% 
#   mutate(yday = yday(survey_date))

sealice_field_data_2019 <- gs_read(field_2019, ws = "sealice_field_data")

fish_and_sealice_field_data_2019 <- left_join(survey_seines_fish_2019, sealice_field_data_2019, by = "ufn") %>%
  filter(field_lice_id_protocol != "NA") %>%
  mutate(all_lice = cal_mot + cpaf + caf + cgf + lpam + lpaf + lam + laf + lgf,
         motile_caligus = cal_mot + cpaf + caf + cgf,
         motile_lep = lpam + lpaf + lam + laf + lgf) %>% 
  select(ufn, survey_date, site_id, region, species, year, all_lice, motile_caligus, motile_lep) %>% 
  gather(`motile_caligus`,
         `motile_lep`,
         `all_lice`,
         key = louse_species,
         value = n_lice) %>% 
  select(ufn, survey_date, site_id, region, species, louse_species, n_lice, year)


#### end 2019 update section and import past years' data from the data package
sealice_field <- read_csv("https://raw.githubusercontent.com/HakaiInstitute/jsp-data/master/data/sealice_field.csv")
...
sealice_lab_mot <- read_csv("https://raw.githubusercontent.com/HakaiInstitute/jsp-data/master/data/sealice_lab_mot.csv")
sealice_lab_fs_mot <- read_csv("https://raw.githubusercontent.com/HakaiInstitute/jsp-data/master/data/sealice_lab_fs.csv") %>% 
...
fish_field_data <- read_csv("https://raw.githubusercontent.com/HakaiInstitute/jsp-data/master/data/fish_field_data.csv", guess_max = 10000)

lab_lice <-
  inner_join(survey_seines, fish_field_data, by = "seine_id") %>%
...
field_lice <-
  left_join(survey_seines, fish_field_data) %>%
...
sealice_time_series <- motile_lice %>%
  ...
  bind_rows(fish_and_sealice_field_data_2019) %>% 
  #TODO: Consider whether these are the appropriate sites
  filter(
    site_id %in% c(
      "D07",
      "D09",
...
saveRDS(sealice_summary_table,  here::here("data", "sealice_summary_table.RDS"))
```
...
saveRDS(lice_bootstraps,  here::here("data", "lice_bootstraps.RDS"))
beepr::beep(8)
```
...
```{r Migration Timing}
#TODO: better deal with doubled intensity during peak this year, rather than simply removing extra seines as done below

# Manually remove additional seines conducted during doubled effort period of peak migration

# I first take Cumulative abundance

tidy_catch <- survey_seines %>%
  #remove doubled up seines
  filter(!survey_id %in% c('DE610', 'DE611', 'DE612', 'DE613', 'DE615','DE624', 'DE625', 'DE626', 'DE627')) %>% 
  select(
    survey_date,
    seine_id,
    region,
...
saveRDS(predict_average_prop, here::here("data", "predict_average_prop.RDS"))
...
saveRDS(cu_cum_abund_annual_DI_2017, here::here("data", "cu_cum_abund_annual_DI_2017.RDS"))          
...
saveRDS(cu_cum_abund_2017, here::here("data", "cu_cum_abund_2017.RDS"))
...
so_predict_annual_prop_DI_2019 <-
  log_cumul_abund(so_cum_abund_annual_DI_2019$percent,
...
pi_predict_annual_prop_DI_2019 <-
  log_cumul_abund(pi_cum_abund_annual_DI_2019$percent,
...
cu_predict_annual_prop_DI_2019 <-
  log_cumul_abund(cu_cum_abund_annual_DI_2019$percent,
...
survey_seines_fish_2019 <- left_join(survey_seines_2019, fish_field_data_2019) %>% 
  select(survey_date = date, region, species, weight, fork_length) %>%
...
length_histo <- left_join(survey_seines, fish_data) %>%
  ...
  mutate(year = as.factor(year)) %>% 
  bind_rows(survey_seines_fish_2019) %>% 
  select(-weight)

saveRDS(length_histo, here::here("data", "length_histo.RDS"))
write_csv(length_histo, here::here("data", "length_histo.csv"))

#TODO: Update so length and weight data come from the JSP database not EIMS Data Portal
...
# Change date column title so I can match up columns
survey_seines_fish_2019 <- survey_seines_fish_2019 %>% 
  rename(date = survey_date) %>% 
  select(-region)
# Make a data request for fish data
...
saveRDS(fish_cond, here("data", "fish_cond.RDS"))
```
...
#create table for predictions from loess function
sim_temp_data_qu39 <-
...
# Create a linear interpolation of points that have zero difference between loess model and 'observed data' so that an area plot will look right
...
qu39_temp_anomaly_data <-
  right_join(sim_temp_data_qu39, qu39_this_year) %>%
  ...
  add_row(
    station = "QU39",
    yday = (176 + 190) / 2,
    predicted_mean_temp = predict(temp.lo_qu39, (176 + 190) / 2),
    mean_temp = predict(temp.lo_qu39, (176 + 190) / 2)
  ) %>% 
  add_row(
    station = "QU39",
    yday = (205 + 211) / 2,
    predicted_mean_temp = predict(temp.lo_qu39, (205 + 211) / 2),
    mean_temp = predict(temp.lo_qu39, (205 + 211) / 2)
  ) 
...
temp.lo_qu29 <- 
  loess(mean_temp ~ yday, qu29_average, SE = T, span = 0.35)
...
qu29_temp_anomaly_data <-
  right_join(sim_temp_data_qu29, qu29_this_year) %>%
  mutate(diff = if_else(mean_temp > predicted_mean_temp, "pos", "neg"))
...
temp.lo_js2_12 <-
  loess(mean_temp ~ yday, js2_12_average, SE = T, span = 0.35)
...
js2_12_temp_anomaly_data <-
  right_join(sim_temp_data_js2_12, js2_12_this_year) %>%
  mutate(diff = if_else(mean_temp > predicted_mean_temp, "pos", "neg")) %>%
   add_row(
    station = "js2_12",
    yday = (130 + 135) / 2,
    predicted_mean_temp = predict(temp.lo_qu39, (130 + 135) / 2),
    mean_temp = predict(temp.lo_qu39, (130 + 135) / 2)
  ) %>%
    add_row(
    station = "js2_12",
    yday = (135 + 142) / 2,
    predicted_mean_temp = predict(temp.lo_qu39, (135 + 142) / 2),
    mean_temp = predict(temp.lo_qu39, (135 + 142) / 2)
  ) %>%
    add_row(
    station = "js2_12",
    yday = (194 + 195) / 2,
    predicted_mean_temp = predict(temp.lo_qu39, (194 + 195) / 2),
    mean_temp = predict(temp.lo_qu39, (194 + 195) / 2)
  )
...
sst_annual_mean$sst_z <- (sst_annual_mean$mean_temp - sst_mean) / sst_sd
...
sst_anomaly_table <- tibble(year = c(2015:current_year),
                            ...
                            Z = round(sst_annual_mean$sst_z, 2))
